...
| Win32 native controls | implemented, never run | `scripts/check_win32.sh` cross-compiles both halves against real Windows headers and checks every symbol in both directions. No image exists for two compiler-repo reasons, neither of them a port: stage1's target predicates follow the host rather than `-target-triple` (it exports `ELISA_HOST_LINUX`, with no Windows equivalent), so a Windows triple compiles the macOS branches instead of the `VirtualAlloc`/`CRITICAL_SECTION`/`GetActiveProcessorCount` ones the runtime already has; and `debug_referee.elisa` declares `kill`/`sigaction` unguarded. |
