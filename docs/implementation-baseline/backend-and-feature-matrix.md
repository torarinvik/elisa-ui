...
| WasmBrowser hosted | implemented-tested for fresh package/inspect; typed runtime smoke externally blocked | `scripts/check_wapp.sh` builds the package from this tree and inspects its profile/imports/exports and JS-freeness. Its WasmBrowser `wb-runtime` Elisa SDK smoke now reaches the correct Rust workspace but fails before guest `start` when Wasmtime converts the SDK component's raw input exports to the WIT-typed binding (`failed to convert function to given type`). A full window/device launch and app-specific host presentation remain separate acceptance work. |
