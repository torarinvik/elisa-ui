...
| Remote rendering/input | implemented-tested at Elisa policy boundary; transport integration planned | `test/remote_test.elisa` covers typed-command/framebuffer/video negotiation, bounded scale conversion, lifecycle generations/overlays, input sequencing/acknowledgements, and generation-bound complete semantic-snapshot admission headlessly. A future WasmBrowser/SDK adapter still owns transport and presentation objects. |
