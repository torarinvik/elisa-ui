...
| WasmBrowser hosted | implemented-tested for fresh package/inspect plus typed hosted runtime smoke | `scripts/check_wapp.sh` builds the package from this tree, inspects its profile/imports/exports and JS-freeness, then runs the WasmBrowser `wb-runtime` Elisa SDK smoke from the correct Rust workspace. A full window/device launch and app-specific host presentation remain separate acceptance work. |
