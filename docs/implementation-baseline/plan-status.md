...
| UI-06 | implemented-tested (partial native collection mapping) | `UiCore` semantics incl. Image/Group/List/Status/Alert and typed collection count/position; `accessibility_metadata_test`, `accessibility_geometry_test`; `UiVirtualAccessibility::Provider` validates and snapshots app-owned off-screen row metadata; AppKit graph bridge exposes full logical list count plus realized visible row proxies, including a million-item/two-proxy fixture | hosted semantics has no WasmBrowser host channel (WB-05); UIKit lazy logical-row container wiring, off-screen focus/scroll navigation, and physical screen-reader runs remain unverified |
| UI-07 | implemented-tested (partial) | sizer/layout corpus, `UiConstraints`, `UiTheme`, `UiLocalization`, `UiValidation`, `UiIdentity`, `UiVirtualList` geometry and bounded realization; `UiHandles::virtual_list` connects logical anchors to retained layout, wheel input, scrollbar drag, row recycling, typed position/count semantics and stale-press protection; `UiVirtualAccessibility::Provider` supplies validated logical row content without materializing widgets; the real Skia Showcase loads, scrolls to, selects, hides and restores item 1,000,000 without growing the widget tree, then verifies identical tail-frame pixels in fresh processes; M5 benchmark retains the policy-window workload; AppKit publishes full logical count plus only realized row proxies | AppKit off-screen focus-and-scroll traversal; UIKit lazy container/provider wiring and logical activation transaction; variable-height/horizontal virtualization; hosted collection mapping; budgets on other reference devices |
