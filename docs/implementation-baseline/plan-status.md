...
| UI-01 | implemented-tested (native; hosted build blocked by compiler) | `UiHandles` builders/stale checks, compatibility entrypoints, generated SDK host bindings; native hello and a fresh `check_wapp.sh` attempt | `wasm-component-ld` 0.5.30 is installed but not on the default `PATH`; after adding its directory, component runtime compilation fails because stage1 emits malformed `llvm.wasm.memory.grow.i32.i32` LLVM IR. See [current validation](current-validation.md#hosted-open-vertical-slice-gate-2026-09-15). |
