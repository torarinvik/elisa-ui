...
| Optional `appkit_skia_host.cpp` compositor | The host receives a borrowed CGContext, creates a temporary Skia surface, and copies pixels back for AppKit presentation. Elisa still prepares/replays the frame and owns renderer selection; the bridge is linked only by the Skia custom-canvas product. |
