...
if ! grep -Eq 'void elisa_appkit_init\(void\)' "$ROOT/src/platform/appkit/appkit_shim.m" || \
   ! grep -Eq 'elisa_appkit_init\(\)' "$ROOT/src/platform/appkit/ui_appkit_native.elisa"; then
...
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_window_style_titled$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_window_style_closable$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_window_style_miniaturizable$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_window_style_resizable$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_window_style_utility$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_bezel_style_rounded$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_button_type_push_on_push_off$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_control_state_on$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_control_state_off$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_progress_style_bar$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_backing_store_buffered$'
nm -g "$ROOT/build/appkit_check" | grep -q ' [BSD] _elisa_appkit_activation_policy_regular$'
...
nm -g "$ROOT/build/appkit_check" | grep -q ' T _elisa_appkit_set_progress_state$'
...
nm -g "$ROOT/build/appkit_check" | grep -q ' T _elisa_appkit_progress_min$'
nm -g "$ROOT/build/appkit_check" | grep -q ' T _elisa_appkit_progress_max$'
nm -g "$ROOT/build/appkit_check" | grep -q ' T _elisa_appkit_progress_value$'
