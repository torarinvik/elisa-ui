#include "../src/platform/common/utf8_utf16.h"
...
int main(void) {
    ...
    uint16_t units[16] = {0};
    size_t unit_count = elisa_utf8_to_utf16(text, sizeof(text), units, 16);
    ...
    size_t byte_count = elisa_utf16_to_utf8(units, unit_count, roundtrip, sizeof(roundtrip));
    ...
    unit_count = elisa_utf8_to_utf16(nul_text, sizeof(nul_text), units, 16);
    CHECK(unit_count == 3 && units[0] == 'A' && units[1] == 0 && units[2] == 'B', "embedded NUL to UTF-16");
    byte_count = elisa_utf16_to_utf8(units, unit_count, roundtrip, sizeof(roundtrip));
    ...
    byte_count = elisa_utf16_to_utf8(lone_surrogates, 3, roundtrip, sizeof(roundtrip));
    ...
    unit_count = elisa_utf8_to_utf16(invalid_utf8, sizeof(invalid_utf8), units, 16);
    ...
    CHECK(elisa_utf8_to_utf16(emoji, sizeof(emoji), units, 1) == 0,
          "UTF-8 to UTF-16 capacity never splits a surrogate pair");
    CHECK(elisa_utf16_to_utf8(expected_units + 3, 2, roundtrip, 3) == 0,
