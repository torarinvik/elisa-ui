...
/* The FIRST rounded gradient of a frame, not the last. A raised surface's
   sheen is a rounded gradient too and it is drawn after the fill, so "last"
   can never tell a ramped fill from a flat one. */
static int first_ramp_red;
static int first_ramp_green;
static int first_ramp_blue;
static int first_ramp_end_red;
static int first_ramp_seen;
static int first_ramp_horizontal;
static int gradient3_count;
static int halo_count;
static int inner_shadow_count;
static int tiled_count;
static float peak_tracking;
static float last_inner_offset_y;
static float last_gradient3_mid_at;
...
void elisa_skia_canvas_fill_round_rect_gradient(size_t canvas, float x, float y, float width,
    ...
    (void)start_alpha; (void)end_green; (void)end_blue; (void)end_alpha;
    if (canvas != 0) {
        if (!first_ramp_seen) {
            first_ramp_seen = 1;
            first_ramp_red = start_red;
...
void skia_test_reset(void) {
    ...
    last_shadow_offset_x = 0.0f; last_shadow_offset_y = 0.0f; last_shadow_blur = 0.0f; last_shadow_red = 0; last_shadow_green = 0; last_shadow_blue = 0; last_shadow_alpha = 0; first_ramp_seen = 0; first_ramp_horizontal = 0; gradient3_count = 0; last_gradient3_mid_at = 0.0f; halo_count = 0; inner_shadow_count = 0; tiled_count = 0; peak_tracking = 0.0f; last_inner_offset_y = 0.0f; first_ramp_red = 0; first_ramp_green = 0; first_ramp_blue = 0; first_ramp_end_red = 0;
...
int skia_test_first_ramp_red(void) { return first_ramp_red; }
...
int skia_test_gradient3_count(void) { return gradient3_count; }
int skia_test_halo_count(void) { return halo_count; }
int skia_test_inner_shadow_count(void) { return inner_shadow_count; }
int skia_test_tiled_count(void) { return tiled_count; }
float skia_test_peak_tracking(void) { return peak_tracking; }
void elisa_skia_set_text_tracking(float tracking) { if (tracking > peak_tracking) peak_tracking = tracking; }
void elisa_skia_canvas_draw_image_tiled(size_t canvas, size_t image, float x, float y, float width, float height, float radius, uint8_t alpha) {
    (void)image; (void)x; (void)y; (void)width; (void)height; (void)radius; (void)alpha;
    if (canvas != 0) { tiled_count += 1; }
}
float skia_test_last_inner_offset_y(void) { return last_inner_offset_y; }
void elisa_skia_canvas_inner_shadow_round_rect(size_t canvas, float x, float y, float width, float height, float radius,
                                               float offset_y, float blur, uint8_t red, uint8_t green, uint8_t blue, uint8_t alpha) {
    (void)x; (void)y; (void)width; (void)height; (void)radius; (void)blur; (void)red; (void)green; (void)blue; (void)alpha;
    if (canvas != 0) { inner_shadow_count += 1; last_inner_offset_y = offset_y; }
}
void elisa_skia_canvas_draw_text_halo(size_t canvas, size_t font, const char *text, size_t length, float x, float y,
                                      float size, float weight_stroke, uint8_t red, uint8_t green, uint8_t blue, uint8_t alpha) {
    (void)font; (void)text; (void)length; (void)x; (void)y; (void)size; (void)weight_stroke; (void)red; (void)green; (void)blue; (void)alpha;
    if (canvas != 0) { halo_count += 1; }
}
float skia_test_last_gradient3_mid_at(void) { return last_gradient3_mid_at; }
void elisa_skia_canvas_fill_round_rect_gradient3(size_t canvas, float x, float y, float width, float height,
                                                 float radius, uint32_t start, uint32_t mid, float mid_at,
                                                 uint32_t end, int32_t horizontal) {
    (void)x; (void)y; (void)width; (void)height; (void)radius; (void)start; (void)mid; (void)end; (void)horizontal;
    if (canvas != 0) { gradient3_count += 1; last_gradient3_mid_at = mid_at; }
}
