...
int main(int argc, char** argv) {
    ...
    for (std::int32_t page = 1; page <= 5; ++page) {
        ...
        if (page == 4) lists_initial_hash = showcase_pixel_region_hash(surface.get());
        std::printf("showcase skia: wrote %s\n", path.c_str());
    }

    // Check the bound picture before benchmark reinitialization deliberately
    // rebuilds the app-owned view and drops this test's injected resource.
    ...
    ShowcaseListBenchmarkResult list_benchmark{};
    if (!benchmark_showcase_list(surface.get(), reinterpret_cast<std::size_t>(typeface.get()),
                                 width, height, lists_initial_hash, &list_benchmark)) return 7;
    const std::string list_tail_path = prefix + "-list-tail.png";
    if (!write_png(surface.get(), list_tail_path)) {
        std::fprintf(stderr, "showcase skia: failed to write %s\n", list_tail_path.c_str());
        return 7;
    }
    std::printf("showcase skia: million-item list tail_pixel_digest=%016llx rendered %s\n",
                static_cast<unsigned long long>(list_benchmark.tail_pixel_digest), list_tail_path.c_str());
    std::printf("{\"workload_id\":\"showcase-million-list-skia-v1\",\"warmups\":%d,\"repetitions\":%d,\"operations_per_batch\":1,\"samples_ns\":[",
                ShowcaseListBenchmarkResult::warmups, ShowcaseListBenchmarkResult::repetitions);
    for (int index = 0; index < ShowcaseListBenchmarkResult::repetitions; ++index) {
        std::printf("%s%llu", index == 0 ? "" : ",",
                    static_cast<unsigned long long>(list_benchmark.samples_ns[index]));
    }
    std::printf("],\"tail_pixel_digests\":[");
    for (int index = 0; index < ShowcaseListBenchmarkResult::repetitions; ++index) {
        std::printf("%s\"%016llx\"", index == 0 ? "" : ",",
                    static_cast<unsigned long long>(list_benchmark.tail_pixel_digest));
    }
    std::printf("]}\nshowcase skia: million-item list benchmark passed\n");
