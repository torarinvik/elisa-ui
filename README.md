...
- **Event ingress** ([src/core/ui_event_queue.elisa](src/core/ui_event_queue.elisa)) —
  ...
  reject ordinary input and expose sticky overflow/drop counts through the
  inspector; lifecycle facts may replace stale physical input so focus loss,
  resize, and quit cannot be stranded behind a burst.
...
- **Text layout** ([src/widgets/ui_text_layout.elisa](src/widgets/ui_text_layout.elisa)) —
  ...
  wrapping advances over the shared `UiText` grapheme walker so combining and
  emoji ZWJ clusters stay intact.
...
large SDK is not available. The gate also runs
...
custom artwork and repeated frame timing on a real off-screen SkSurface.
